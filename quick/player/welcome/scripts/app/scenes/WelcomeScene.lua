...
function WelcomeScene:createButtons()
    ...
    local newProjectButton = cc.ui.UIPushButton.new("NewProjectButton_zh.png")
        :onButtonClicked(function()
            local evt = cc.EventCustom:new("WELCOME_NEW_PROJECT")
            cc.Director:getInstance():getEventDispatcher():dispatchEvent(evt)

            cc.player.trackEvent("newproject")
        end)
        :pos(left, top)
        :addTo(self)
    ...
    local openProjectButton = cc.ui.UIPushButton.new("OpenProjectButton_zh.png")
        :onButtonClicked(function()
            local evt = cc.EventCustom:new("WELCOME_OPEN_PROJECT")
            cc.Director:getInstance():getEventDispatcher():dispatchEvent(evt)

            cc.player.trackEvent("openproject")
        end)
        :pos(left + 200, top)
        :addTo(self)
    ...
    local openCommunityButton = cc.ui.UIPushButton.new("OpenCommunityButton_zh.png")
        :onButtonClicked(function()
            local evt = cc.EventCustom:new("WELCOME_OPEN_COMMUNITY")
            cc.Director:getInstance():getEventDispatcher():dispatchEvent(evt)
            cc.player.trackEvent("community")
        end)
        :pos(left, top - 140)
        :addTo(self)
    ...
    local openDocumentsButton = cc.ui.UIPushButton.new("OpenDocumentsButton_zh.png")
        :onButtonClicked(function()
            local evt = cc.EventCustom:new("WELCOME_OPEN_DOCUMENTS")
            cc.Director:getInstance():getEventDispatcher():dispatchEvent(evt)
            cc.player.trackEvent("documents")
        end)
...
function WelcomeScene:createBannerButton( AD, x)
    local banner = cc.ui.UIPushButton.new(AD.img, {scale9 = true})
                    ...
                    :addTo(self)
                    :onButtonClicked(function() 
                            device.openURL(AD.link) 
                            cc.player.trackEvent("click "..AD.link) 
                        end)
    return banner
end
