...
bool HTTPRequest::initWithUrl(const char *url, int method)
{
        ...
        "(Ljava/lang/String;)Ljava.net.HttpURLConnection"))
    {
        ...
        m_httpConnect = methodInfo.env->CallStaticVoidMethod(methodInfo.classID, methodInfo.methodID, jurl);
    ...
    if (JniHelper::getStaticMethodInfo(methodInfo,
        ...
        "(Ljava/lang/String;)V"))
    {
        jstring jstr = methodInfo.env->NewStringUTF(method);
        m_httpConnect = methodInfo.env->CallStaticVoidMethod(methodInfo.classID, methodInfo.methodID, jstr);
